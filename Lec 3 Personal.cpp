 <--BITSET--> //  used in segment tree
	...
	cout << bt.test(1); // check if the bit is set or not at index  1 // returns bool value
    ...
    *max_element(firstIterator, lastIterator); 
    // has internal implimantation as follows
    int maxi = INT_MIN; 
    for(int k = i;k<=j;k++) {
    	if(a[k] > maxi) {
    		maxi = a[k]; 
    ...
    // arr[] -> {1, 2, 5, 1, 2, 4, 4} 
    auto it = find(arr, arr+n, 2); // return an iterator         // O(N) 
    ...
    bool res = binary_search(arr, arr+n, 8);                   // O(logN) complexity
    ...
    // x = 6 
    // x = 13 
    this works in log N as it uses BINARY search for implementation 
    ...
    // s = "bca" 
    bool res = next_permutation(s.begin(), s.end());  // next_permuation --> O(N) time
    ...
    do {
    	cout << s << endl; 
    } while(next_permutation(s.begin(), s.end())); // while loop runs factorial(N) times as that much permutations are there! // O(factorial(N))
    ...
    bool comp(int el1, int el2) {  // comparator for increasing order
	if(el1 <= el2) {
		return true; 
	}
	return false; 
    ...
    bool comp(int el1, int el2) {  // comparator for decreasing order
	if(el1 >= el2) {
		return true; 
	}
	return false; 
    ...
    sort(arr, arr+3, greater<pair<int, int>>);
